...
const roll = ({ message, player, discordMember, args }: RollParams) => {
	...
	const isSavedRoll = (input: string) =>
		rollRegexes.filter(({ regExp }) => regExp.test(input)).length > 0 &&
		!/#/.test(input);
	/**
	...
	const getRollDetails = (args: string[]) => {
		if (isSavedRoll(args.join(' '))) {
			let [savedRoll] = args;
			let [{ property }] = rollRegexes.filter(({ regExp }) =>
				regExp.test(savedRoll)
			);
			let playerStat = player.stats[property] || 10;
			let mod = player
				? Math.floor((player.stats[property] - 10) / 2)
				: 0;
			return {
				modifiers: mod !== 0 ? [mod] : [],
				rollName: savedRoll,
	...
	const reply = () => {
		...
		let readableModifiers2 = modifiers.map((m) =>
			m > 0 ? `+ ${m} ` : `- ${m * -1} `
		);
		if (modifiers.length > 0) {
			...
			return `\`${rolls[0]}${rollName ? ' #' + rollName : ''}\`: **${
				totalRaw[0]
			}**`;
		}
	};
	if (!message) {
		...
		if (args.join(' ').match(/-s/i))
			message.channel.send(
				`<@${discordMember.id}>: ${totalRaw.reduce(
					(pv, cv) => pv + cv,
					0
				)}`
			);
