...
export const isValidCategory = (input: string) => {
	return categories.includes(input);
};

export const isCoin = (input: string) => {
	return /gold|silver|electrum|platinum|copper/.test(input);
};

export default function (message: Message) {
	/**
	 * Handles sending message based on user's `notificationsToDM` property.
	 * Sends message to either channel or DMs
	 */
	const channelOrDM = (
		player: IPlayer,
		botMessageContents: string | MessageEmbed
	) => {
		try {
	...
	const createInventoryEmbed = (
				...
				case 'inventory':
					...
					return new MessageEmbed()
						...
						.addFields(
							...
							{
								name: 'Last updated',
								value: moment(lastUpdated).format(
									'MMMM Do, hh:mm a'
								),
							}
						)
		...
		return send === 'send'
			? channelOrDM(player, getEmbed())
			: message.author.send(getEmbed());
	};
	return {
