...
function removeItem(message, args, player) {
	...
	if (!cat) {
		...
		createResponseEmbed("send", "invalid", "Remove what? Must specify /remove gold 10, /remove weapon staff, etc...", player);
		return player;
	} else if ((coins.isCoin(cat) && isNaN(removedItem)) || (coins.isCoin(cat) && !removedItem)) {
